  ...
  <div class="container mb-5" style="margin-top:-170px;">
    ...
    <div class="row justify-content-center" v-if="getPromotionObjects[106].status">
      <div class="col-12 col-md-8 px-5">
        <h5 class="text-center font-weight-100 text-white mb-3" style="margin-top: 85px;">ฝากประจำ</h5>
        ...
        <div class="row mt-3" v-for="(row, index) in rows" :key="index">
          <div
            class="col px-1"
            @click="boxConfirm((index * colunms + (colunm)))"
            v-for="(colunm, key) in colunms"
            :key="key"
          >
            <div
              class="card shadow border-0 text-center"
              :class="{'bg-template': (index * colunms + (colunm)) <= countDate , 'push': dailyArray[(index * colunms + (colunm))] }"
            >
              <div class="card-body p-2">
                <h4
                  class="mb-0 font-weight-normal"
                  :class="{'text-muted' : (index * colunms + (colunm)) > countDate }"
                >{{ index * colunms + (colunm) }}</h4>
                <p
                  style="white-space: nowrap;"
                  class="text-warning small"
                  v-if="dailyArray[(index * colunms + (colunm))]<=countDate"
                >
                ...
                <p
                  style="white-space: nowrap;"
                  class="text-warning small"
                  v-else-if="dailyArray[(index * colunms + (colunm))]"
                >
                  {{'รับ ' +dailyArray[(index * colunms + (colunm))].credit}}
                  <small></small>
                </p>
...
export default {
  ...
  asyncData: async function ({ $axios }) {
    ...
    return { dailyArray: dailyDay, countDate: data.count_date };
  },
  methods: {
    acceptDaily: async function (day) {
      const daily = await this.$axios.$post("/api/daily-accecpt", { day: day });
      if (daily.success) {

<template>
  <div class="container mb-5" style="margin-top:-130px;">
    <h6 class="mb-2 text-white" style="text-align: center;">สะสมแต้ม</h6>
    ...
    <div class="row">
      <div class="col">
        <h4 class="mb-0 font-weight-100 text-center text-white">สะสมแต้ม {{ $auth.user.point }} แต้ม</h4>
        ...
        <b-tabs>
          <b-tab no-body title="ของรางวัล">
            <b-card-footer>
              ...
              <div class="form-group float-label active">
                <select
                  @change="searchDetaiItemArray"
                  class="custom-select form-control text-template form-control-lg"
                  ref="searchTypeItemArray"
                >
              ...
              <div class="form-group float-label active">
                <select
                  @change="searchDetaiItemArray"
                  class="custom-select form-control text-dark form-control-lg"
                  ref="searchPointItemArray"
                >
                  ...
                  <option
                    ...
                    :key="index"
                  >{{ item.over ? `มากกว่า ${item.over} คะแนน` : `${item.between.start} - ${item.between.end} คะแนน`}}</option>
                </select>
              ...
              <div class="row" ref="formSearchItem">
                <div
                  class="col-md-6 col-xs-6 mb-3"
                  @click="boxConfirm(item.id)"
                  v-for="(item, index) in itemPagination"
                  :key="index"
                >
                  <img class="img-fluid pointer" :src="showAPI + item.img" :alt="item.detail" />
                </div>
              </div>
              <div class="col-12 mt-3" v-if="searchArray.length">
                <b-pagination
                  align="center"
                  v-model="currentPage"
                  @change="searchDetaiItemArray"
            ...
            <b-card-footer>
              ...
              >
                <div class="card-body">
                  <div class="row">
                    <div class="col">
                      <h5 class="font-weight-normal mb-1 text-warning">{{item.detail}}</h5>
                      <p class="small text-secondary mb-2">
                        <span
                          class="badge small vm text-template"
                          :class="[{ 'badge-info' :  item.accept_status == 0 ,'badge-success' :  item.accept_status == 1 ,'badge-danger' :  item.accept_status != 0 && item.accept_status != 1 } ] "
                        >{{ toStatusString(item.accept_status) }}</span>
                      </p>
                      <p class="text-mute small text-secondary mb-2">{{item.create_at}}</p>
                      <div class="progress h-4">
                        <div
                          class="progress-bar"
                          :class="[{ 'bg-info w-50' :  item.accept_status == 0 ,'bg-success  w-100' :  item.accept_status == 1,'bg-danger  w-100' :  item.accept_status != 0 && item.accept_status != 1 }]"
                          role="progressbar"
                          aria-valuenow="25"
                          aria-valuemin="0"
                          aria-valuemax="100"
...
export default {
  ...
  asyncData: async function ({ $axios, $auth }) {
    const { data } = await $axios.$get("/api/reward");
    const historyData = await $axios.$get("/api/player-reward-history", {
      params: { account_api: $auth.user.account_api },
    });
    const typeOptions = [
      { id: 0, between: { start: 1, end: 12000 } },
      { id: 1, between: { start: 12000, end: 24000 } },
      { id: 2, between: { start: 24000, end: 36000 } },
      { id: 3, between: { start: 36000, end: 48000 } },
      { id: 4, between: { start: 48000, end: 60000 } },
      { id: 5, between: { start: 60000, end: 72000 } },
      { id: 6, over: 72000 },
    ];
    if (!historyData) {
      this.$toast.global.error({ message: "ไม่สามารถ..." });
    }
    return {
  ...
  methods: {
    boxConfirm: function (id) {
      ...
      const messageVNode = h("div", { class: ["text-center"] }, [
        h("img", {
          attrs: { class: ["img-fluid mb-3"], src: [this.showAPI + item.img] },
        }),
        h("h5", { class: ["text-center"] }, [`ชื่อของรางวัล ${item.detail}`]),
    ...
    acceptReward: async function (id) {
      ...
      if (success) {
        this.$auth.fetchUser();
        this.$toast.global.success({ message: data });
      } else {

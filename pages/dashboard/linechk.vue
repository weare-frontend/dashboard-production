          ...
          <div class="col-12" id="acceptOtp">
            <b-collapse id="collapse-otp">
              ...
              <a
                class="text-danger"
                v-else
              >จะสามารถลองใหม่อีกครั้งภายใน เวลา {{this.coutDownOtp}} วิ</a>
            </b-collapse>
          </div>
                ...
                <div class="input-wrappe">
                  <client-only placeholder="Loading...">
                    <pincode-input
                      ...
                      class="form-controls text-center"
                      placeholder="_"
                      :autofocus="true"
                    ></pincode-input>
                  </client-only>
...
export default {
  asyncData: async function ({ route, $axios }) {
      ...
      if (!this.inputPincode || this.inputPincode == "") {
        ...
        if (research.success) {
          ...
          return this.$toast.global.error({
            message:
              "ไม่สามารถเชื่อมต่อเซิฟเวอร์ได้ (400) / กรุณาติดต่อพนักงาน",
          });
        }
      }
    },

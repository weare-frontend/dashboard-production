...
<div>
    <!-- <page-info /> -->
    <div class="row">
        <b-modal id="modal-exchange" ref="modal-exchange" centered content-class="rounded" header-class="shadow" footer-class="shadow" no-close-on-backdrop body-class="bg-body-modal">
            <template v-slot:modal-header="{ close }">
                <h5 class="text-center w-100" style="color: #0f4845;">โอนเครดิตฟรีเข้าเกม</h5>
                <i class="f7-icons text-white multiply-circle" @click="$bvModal.hide('modal-exchange')">multiply_circle</i>
            </template>
            <template v-slot:modal-footer="{ cancel }">
                <div class="row justify-content-center">
                    <div class="col-6">
                        <button type="button" class="btn text-white py-0 px-4 bg-button-cc" @click="$bvModal.hide('modal-exchange')">ยกเลิก</button>
                    </div>
                    <div class="col-6">
                        <button type="button" @click="$refs['modal-download'].show()" class="btn btn-block py-0 px-4 btn-outline-secondary bg-button-cf">ยืนยัน</button>
                    </div>
            ...
            <div class="row mt-3 justify-content-center" v-if="$route.name != 'dashboard-withdraw'">
                ...
                <div class="col-4 text-center mb-1">
                    <h6 class="m-0 w-100 mb-2" style="white-space: nowrap;color: #fff;">กระเป๋าเครดิตฟรี</h6>
                    <img class="mb-1" src="~/assets/img/walletnew.png" style="height:80px; padding:1px; " />
            ...
            <b-modal id="modal-download" ref="modal-download" centered content-class="rounded" header-class="shadow" body-class="bg-body-modal" footer-class="shadow" no-close-on-backdrop>
                <template v-slot:modal-header="{ close }">
                    <i class="f7-icons text-white multiply-circle2" @click="$bvModal.hide('modal-download')">multiply_circle</i>
                    <p class="text-center w-100" style="font-weight:100; font-size:18px; color: #0f4845;">ยืนยันถอนเครดิตฟรี</p>
                </template>
                <template v-slot:modal-footer="{ cancel }">
                    <div class="row justify-content-center">
                        <div class="col-6">
                            <button type="button" class="btn text-white py-0 px-4 bg-button-cc" @click="$bvModal.hide('modal-download')">ยกเลิก</button>
                        </div>
                        <div class="col-6">
                            <button type="button" @click="exchangeCredit" class="btn btn-block btn-outline-secondary py-0 px-4 bg-button-cf">ยืนยัน</button>
                        </div>
                ...
                <div class="row justify-content-center" v-else>
                    ...
                    <div class="col-12 text-center text-template mb-2" style="margin-bottom: -5px;">
                        <label style="color: white;">คุณต้องการแลกเครดิตฟรีใช่หรือไม่ ?</label>
                    </div>
                    <!-- <div class="col-4">
              <button
                type="button"
            ...
            <div class="row justify-content-center">
                <div class="col-9 mt-1">
                    <div class="form-group mx-3">
                        ...
                        <input type="number" ref="exchange" placeholder="จำนวนเงิน" class="text-center text-template form-control rounded" />
                    </div>
                </div>
            </div>
            ...
            <button class="btn btn-default btn-rounded-54 shadow bg-template" @click="fetchUser">
              ...
              >autorenew</i>
                ...
                <div class="mb-2 d-flex align-items-center">
                    <div class="card card-body mx-4 pb-2 shadow" ref="formInfoUser">
                        <div class="row justify-content-center">
                            <div class="fetch justify-content-center">
                            ...
                            <div class="col-12 mb-0 mt-2 text-center">
                                <span class="text-template">
                                    ...
                                    <h4 class="text-template font-weight-100 size-22">{{ $auth.user.money+'฿' }}</h4>
                                </span>
                                <!-- <button class="btn small material-icons text-template size-10 radius-20">เติมเงินนอัตโนมัติ</button> -->
                            </div>
                            <div class="col-12 text-center pt-2" style="border-top: 1px solid #e4e9f0;">
                                <div class="row text-template justify-content-center d-flex align-items-center ">
                                    <div class="bg-template" style="position:absolute;width:60px;left:10%; border-radius: 50%;">
                                        <img src="~/assets/icon/Wallet-revert.png" class="w-100" style="filter: invert(1);" />
                                    </div>
    ...
    <div class="container-sm" style=" max-width: 500px;">
        <div class="row text-center pt-2 mt-3 justify-content-center" style="background: #fff; border-bottom-right-radius: 20px; border-top-right-radius: 20px; margin-right: 10px; padding-left: 20px; filter: drop-shadow(0px 1px 1px #25968f);">
            <div class="col-3" id="btn-topup" style="background: linear-gradient(180deg, #ff975600 36%, #cbf594 100%);">
                        ...
                        <div class="text-center">
                            <div class="border-0">
                                <div class="overlay bg-template mb-0" style="margin:auto;">
                                    ...
                                    <img src="~/assets/icon/Deposit-revert.png" class="icon-template" />
                                </div>
                            </div>
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">เติมเงิน</small>
                ...
                <nuxt-link :to="{name : 'dashboard-withdraw'}">
                    <div class="border-0 mb-0">
                        <div class="text-center">
                            <div class="border-0">
                                <div class="overlay bg-template mb-0" style="margin:auto;">
                                    <img src="~/assets/icon/Withdraw-revert.png" class="icon-template" />
                                </div>
                            </div>
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">ถอนเงิน</small>
                ...
                <nuxt-link :to="{name : 'dashboard-promotions' }">
                    <div class="border-0 mb-0">
                        <div class="text-center">
                            <div class="border-0">
                                <div class="overlay bg-template mb-0" style="margin:auto;">
                                    <img src="~/assets/icon/Promotion-revert.png" class="icon-template" />
                                </div>
                            </div>
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">โปรโมชั่น</small>
                            <!-- <p class="text-secondary text-mute small">แสดงรายชื่อเพื่อนที่ชวนทั้งหมด</p> -->
                        </div>
                    </div>
                </nuxt-link>
            </div>
            <div class="col-3" id="btn-referral" v-if="getPromotionObjects[121] && getPromotionObjects[121].status">

                <div class="caadow border-0 mb-0" @click="affiliate()">
                    <div class="text-center">
                        <div class="border-0">
                            <div class="overlay bg-template mb-0" style="margin:auto;">
                                <img src="~/assets/icon/Affilate-revert.png" class="icon-template" />
                            </div>
                        </div>
                        <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">คอมมิชชั่น</small>
                    </div>
                </div>
            </div>
                ...
                <nuxt-link :to="{name : 'dashboard-affiliate' }">
                    <div class="border-0 mb-0">
                        <div class="text-center">
                            ...
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">ชวนเพื่อน</small>
        ...
        <div class="row text-center justify-content-center mt-3 pt-2" style="background: #fff; border-bottom-left-radius: 20px; border-top-left-radius: 20px; margin-left: 5px; padding-right: 25px;filter: drop-shadow(0px 1px 1px #25968f);">
            <div class="col-3" id="btn-reward" v-if="getPromotionObjects[102].status">
                <nuxt-link :to="{name : 'dashboard-point'}">
                    <div class="border-0 mb-0">
                        <div class="text-center">
                            <div class="border-0">
                                <div class="overlay bg-template mb-0" style="margin:auto;">
                                    <img src="~/assets/icon/Reward-revert.png" class="icon-template" />
                                </div>
                            </div>
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">สะสมแต้ม</small>
            ...
            <div class="col-3" id="btn-reward" v-else>
                <div class="caadow border-0 mb-0">
                    <div class="text-center">
                        <div class="border-0">
                            <div class="overlay bg-template mb-0" style="filter:brightness(0.4)">
                                <img src="~/assets/icon/Reward-revert.png" class="icon-template" />
                            </div>
                        </div>
                        <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">สะสมแต้ม</small>
            ...
            <div class="col-3" id="btn-topup-daily" v-if="getPromotionObjects[106].status">
                <nuxt-link :to="{name : 'dashboard-daily'}">
                    <div class="border-0 mb-0">
                        <div class="text-center">
                            <div class="border-0">
                                <div class="overlay bg-template mb-0" style="margin:auto;">
                                    <img src="~/assets/icon/Daily-revert.png" class="icon-template" />
                                </div>
                            </div>
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">ฝากประจำ</small>
                            <!-- <p class="text-secondary text-mute small">สามารถรับของรางวัลต่าง ๆ ได้เมื่อฝากทุกวัน</p> -->
                        </div>
                    </div>
                </nuxt-link>
            ...
            <div class="col-3" id="btn-luckywheel" style="background: linear-gradient(180deg, #ff975600 36%, #cbf594 100%);" v-if="getPromotionObjects[111].status">
                <nuxt-link :to="{name : 'dashboard-activity-wheel' }">
                    <div class="caadow border-0 mb-0">
                        <div class="text-center">
                            <div class="border-0">
                                <div class="overlay bg-template mb-0" style="margin:auto;">
                                    <img src="~/assets/icon/Wheel-revert.png" class="icon-template" />
                                </div>
                            </div>
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">วงล้อลุ้นโชค</small>
                            <!-- <p class="text-secondary text-mute small">กดแชร์ facebook รับครดิตทันที</p> -->
                        </div>
                    </div>
                </nuxt-link>
            ...
            <div class="col-3" id="btn-activity" v-if="this.getPromotionObjects[111].status || this.getPromotionObjects[103].status || this.getPromotionObjects[116].status || this.getPromotionObjects[126].status">
                <nuxt-link :to="{name : 'dashboard-activity' }">
                    <div class="caadow border-0 mb-0">
                        <div class="text-center">
                            <div class="border-0">
                                <div class="overlay bg-template mb-0" style="margin:auto;">
                                    <img src="~/assets/icon/Event-revert.png" class="icon-template" />
                                </div>
                            </div>
                            <small style="white-space: nowrap;" class="text-template mb-0 font-weight-100 size-14">กิจกรรม</small>
                            <!-- <p class="text-secondary text-mute small">กดแชร์ facebook รับครดิตทันที</p> -->
                        </div>
                    </div>
                </nuxt-link>
        ...
        <div class="row py-3 mb-5">
            <div style="width:100%;margin-left: auto; margin-right: auto;">
                <carousel-3d :display="3" :autoplay="true" :autoplay-timeout="2400" :height="360" class="width c-bill" style="background:rgba(0,0,0,0);border-radius:20px; border:0;">
                    <slide v-for="(item, i) in promotionArray" :index="i" :key="i">
                        <img :src="item.img" class="w-100" alt="promotion" style="border-radius:20px; overflow: hidden;" />
                    </slide>
                </carousel-3d>
            </div>
        </div>
...
.bg-button-cc {
    background: linear-gradient(to right, rgb(129 224 216 / 1) 0%, rgb(0 0 0 / 0%) 100%);
    background-color: #4EA998;
    color: #fff;
}
    ...
    methods: {
        ...
        exchangeCredit: async function () {
            ...
            const {
                success,
                message
            } = await this.$axios.$post("/api/topup-bonus", {
